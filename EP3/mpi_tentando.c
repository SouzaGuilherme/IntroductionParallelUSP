...
static void master(int num_process, int width, int height){
    ...
    for(int p = 0; p < num_process; p++){
        tasks[p].vector = malloc(width*height*sizeof(int));
        if(tasks[p].vector == NULL)
            DIE("Err, viado\n");
    }
    int err = 0;
    ...
    for (int i = 0; i < num_process; ++i){
        ...
        //tasks[i].vector = buffer_master;
        printf("start ==> %d\n", tasks[i].start);
        printf("end ==> %d\n", tasks[i].end);
        printf("Inicial vector ==> %d\n", tasks[i].vector[0]);

        err |= MPI_Send(&tasks[i],   // Buffer to send
                sizeof(tasks[i]), // How many elements. Note that there is a hack here.
    ...
    // Gather the results
    printf("PEGA OS RESULTADOS DE VOLTA\n");

    int *buffer_PEGA = malloc(width*height*sizeof(int));
    if(buffer_PEGA == NULL)
        DIE("Err, buffer_PEGA\n");
    ...
    for(int i = 0; i < num_process; i++){
        ...
        for(int b = acess_buffer.start; b < acess_buffer.end; b++){
            int d = b / width;
            int j = b % width;

            printf("acess_buffer[].vector[] ==> %d\n", acess_buffer.vector+(d*width+j));
            buffer_PEGA[d*width+j] = acess_buffer.vector+(d*width+j);
            printf("buffer_master[] ==> %d\n",buffer_PEGA+(d*width+j));
        }
    }
}

void calc_mandelbrot(struct task *acess, int width, int height, float min_real, float min_imag, float max_real, float max_imag){
    printf("MANDELBROT\n");
    for(int i = acess->start; i < acess->end; i++){
        ...
        printf("ITERATION ==> %d\n", iteration);
        printf("ANTES -> BUFFER[] ==> %d \n", (acess+i)->vector+(width*d+j));
        *((acess+i)->vector+(width*d + j)) = iteration;
        printf("DEPOIS -> BUFFER[] ==> %d \n", (acess+i)->vector+(width*d+j));
...
void slave(int task_id, int width, int height, float min_real, float min_imag, float max_real, float max_imag){
    ...
    /* Calc of Mandelbrot */
    calc_mandelbrot(&recv, width, height, min_real, min_imag, max_real, max_imag);
