...
void master(int num_process, int width, int height, char file_name[]){
    int err = 0;
    ...
    int result = writeImage(file_name, width, height, buffer);
    
    free(buffer);
};

/* Calc_mandelbrot sequential */
void calc_mandelbrot_seq(int start, int end, int *vetor, int width, int height, float min_real, float min_imag, float max_real, float max_imag){
    int correct_vetor = 0;
...
void calc_mandelbrot_mult(int start, int end, int *vetor, int width, int height, float min_real, float min_imag, float max_real, float max_imag){
...
void slave(int task_id, int width, int height, float min_real, float min_imag, float max_real, float max_imag, int threads, int flag_accelerator){
    int err = 0;
    struct task *recv;
    err |= MPI_Recv(recv,
            sizeof(struct task),
	...
	if(flag_accelerator == 0){

		calc_mandelbrot_seq(recv->start, recv->end, vetor, width, height, min_real, min_imag, max_real, max_imag);
	}else if(flag_accelerator == 1){

		calc_mandelbrot_mult(recv->start, recv->end, vetor, width, height, min_real, min_imag, max_real, max_imag);

	}else if(flag_accelerator == 2){
		int val_temp = recv->start;	
		for(int i = 0; i < tam_vetor; i++){
			vetor[i] = val_temp+i;
		}

		invoke_cuda(tam_vetor, width, height, &min_real, &min_imag, &max_real, &max_imag, threads, vetor);
	}
...
int main(int argc, char *argv[]){
    ...
    if((strcmp(accelerator, "CPU") == 0) || (strcmp(accelerator, "cpu") == 0)){
	    if(threads > 1){
		    printf("Process Image in CPU PARALLEL\n");
    ...
    if(task_id == 0){
        master((num_process-1), width, height, file_name);
    
    }else if(task_id <= (num_process-1)){
        printf("FILHO\n");
        slave(task_id, width, height, min_real, min_imag, max_real, max_imag, threads, flag_accelerator);
    }
